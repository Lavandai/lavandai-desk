...
console.log("✅  API Key presente:", API_KEY.slice(0, 18) + "...");
...
function proxyChat(req, res) {
  ...
  req.on("end", () => {
    ...
    const proxyReq = https.request(options, (proxyRes) => {
      console.log(`[Anthropic] ${proxyRes.statusCode}`);
...
const server = http.createServer((req, res) => {
  ...
  console.log(`[ROUTER] url="${url}" method=${req.method}`);

  // Health check
  if (url === "/health") {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ status: "ok", key: API_KEY.slice(0, 10) + "..." }));
    return;
  }

  // Fornisce la chiave al frontend in modo sicuro (solo dalla stessa origine)
  if (url === "/apikey") {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ key: API_KEY }));
    return;
  }
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`\n🫧  LavandAI Desk avviato su porta ${PORT}\n`);
});
